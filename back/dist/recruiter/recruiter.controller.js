"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.RecruiterController = void 0;
const common_1 = require("@nestjs/common");
const user_decorator_1 = require("../decorators/user.decorator");
const recruiter_login_dto_1 = require("./dto/recruiter-login.dto");
const jwt_auth_guard_1 = require("../user/guards/jwt-auth.guard");
const recruiter_SignUp_dto_1 = require("./dto/recruiter-SignUp.dto");
const recruiter_service_1 = require("./recruiter.service");
let RecruiterController = class RecruiterController {
    constructor(recruiterService) {
        this.recruiterService = recruiterService;
    }
    SignUpRecruiter(recruiterinfo) {
        return this.recruiterService.SignUpRecruiter(recruiterinfo);
    }
    Login(userlogin) {
        return this.recruiterService.login(userlogin);
    }
    findAll(user) {
        return this.recruiterService.findAll(user);
    }
    async findById(id) { return this.recruiterService.getUserById(id); }
};
__decorate([
    (0, common_1.Post)('register'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [recruiter_SignUp_dto_1.RecruiterSignUpDto]),
    __metadata("design:returntype", Promise)
], RecruiterController.prototype, "SignUpRecruiter", null);
__decorate([
    (0, common_1.Post)('login'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [recruiter_login_dto_1.RecruiterLoginDto]),
    __metadata("design:returntype", void 0)
], RecruiterController.prototype, "Login", null);
__decorate([
    (0, common_1.Get)('show'),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    __param(0, (0, user_decorator_1.User)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], RecruiterController.prototype, "findAll", null);
__decorate([
    (0, common_1.Get)('showById/:id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number]),
    __metadata("design:returntype", Promise)
], RecruiterController.prototype, "findById", null);
RecruiterController = __decorate([
    (0, common_1.Controller)('recruiter'),
    __metadata("design:paramtypes", [recruiter_service_1.RecruiterService])
], RecruiterController);
exports.RecruiterController = RecruiterController;
//# sourceMappingURL=recruiter.controller.js.map